...
public final class SignChangeListener implements Listener {
	...
	public void onSignChange(SignChangeEvent event) {
		...
		if (event.getLine(0).contains(plugin.getConfig().getString("signTags.rent"))) {
			...
			// If the secondLine does not contain a name try to find the region
			// by location
			...
			if (!player.hasPermission("areashop.createrent.others")) {
			...
			if (!player.hasPermission("areashop.createbuy.others")) {
				ProtectedRegion region = regionManager.getRegion(secondLine);
				if (region == null || !region.getOwners().contains(player.getUniqueId())) {
					plugin.message(player, "setup-noPermissionBuy");
					return;
